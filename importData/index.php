  ...
  ENGINE = InnoDB;");
    ...
    if (($handle = fopen ($movimentos_csv[0], 'r' )) !== FALSE) {
        while (($data = fgetcsv($handle, 10000000, ',' )) !== FALSE ) {
            //print_r($data);die();
            $sql_p = "INSERT INTO movimento(id,designacao,tipo,produto_id,cliente_id,valor,data) 
            VALUES(".$data[0].",'".escape($data[1])."','".$data[2]."','".$data[3]."','".$data[4]."','".$data[5]."','".$data[6]."')
            ";
            mysqli_query($con, $sql_p);
        }
    ...
    if (($handle = fopen ($clientes_csv[0], 'r' )) !== FALSE) {
        while (($data = fgetcsv($handle, 10000, ',' )) !== FALSE ) {
            ...
            $sql_p = "INSERT INTO cliente(id,Surname,CreditScore,Geography,Gender,DateOfBirth,OpenAccountDate,CurrentAccountNumber,CreditCardAccountNumber,Balance,BalanceDate,EstimatedSalary) 
            VALUES(".$data[1].",'".escape($data[2])."','".$data[3]."','".$data[4]."','".$data[5]."','".$d1."','".$d2."','".$data[8]."','".$data[9]."','".$data[10]."','".$d3."','".$data[12]."')
            ";
            mysqli_query($con, $sql_p);
        }
        print 'Done Clientes';
